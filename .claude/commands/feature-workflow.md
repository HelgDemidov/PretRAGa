...
CI exists: `.github/workflows/ci.yml`, one job, per-step statuses. Steps below that read a CI baseline read it from the live CI log of that file.

## Step 1 — Find the target spec

If invoked with an argument, use that spec directly.

Otherwise scan `docs/*/tech_specs/*/spec.md` for specs still awaiting implementation. The spec command writes a fixed status vocabulary near the top: `draft v1` (pending), progressing to `implemented (PR #N)` (set by this command's own final step) — select the ones still pending; treat anything that is not the terminal value as pending. Umbrella documents matching `docs/*/charters/*.md` are intentionally NOT matched by the scan, and neither is `docs/backlog/backlog.md`, which is excluded by file name rather than by location. **As of 2026-08-01 no spec exists yet**, so the zero-match branch below is the expected one.
...
- **The full gate at most once per commit**, immediately before committing:
  ...
  The canonical command lives in `CLAUDE.md` §10 and mirrors `.github/workflows/ci.yml`
  byte for byte; on divergence reconcile against ci.yml. It is not copied here —
  a copy would drift.

The gate's exclusion filter (`not heavy`): today the single hand-applied `heavy` marker suffices; when toolchain-gated labels appear, apply CLAUDE.md §10's umbrella-label rule (one collection hook, not N copies of the filter). The once-per-commit budget matters on a constrained machine: use the full gate as the pre-commit checkpoint, not as a companion to every edit.
