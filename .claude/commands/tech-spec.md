...
Every spec is checked against these in Step 3. They are not invented here: each restates, as a spec-time check, a rule that already binds this repository — `CLAUDE.md` §4 (data and state modelling) and the design-truth charter, `docs/design_truth/charter/charter.md`.
...
2. **Every layer declares a port, not a union of what its sources happen to provide.** A new module under `src/` defines its own minimal input type — exactly the fields it actually reads — never a shared record type that grows to the union of every consumer's needs. Check: the input type names its fields, and the ring contracts still pass (`PYTHONPATH=src lint-imports`).
...
**Coverage floor:** enforced by CI (`fail_under` in `pyproject.toml` [tool.coverage.report]). Specs state test depth qualitatively and never copy the number — it is a regression floor read from the live config, not a goal.

## Step 1 — Ground the spec in reality

Read: the `src/` modules the spec will touch; the schema or type definitions it extends (new fields must extend them correctly, not duplicate or contradict them); any controlled vocabularies or configuration it depends on; two or three existing specs matching `docs/*/tech_specs/*/spec.md` for tone and structure — **as of 2026-08-01 there are none: no spec has been written yet**, so match the density of `docs/design_truth/charter/charter.md` instead; the root `CLAUDE.md`; the project memory index (`MEMORY.md`) and the entries it links; and `docs/backlog/backlog.md` — a spec often starts as an item there.
...
## Test coverage
<the tests this spec requires, one bullet group per commit in the plan below. Unit tests by default, CI-safe unless explicitly marked; call out where an integration test is the honest way to cover a path a unit test would only fake. State test depth qualitatively; the numeric floor lives in pyproject and is never copied into a spec>
...
Design documents in this repo are written in Russian (`CLAUDE.md` §11 fixes artifact language by class: code is English-only, project documentation is Russian) — section headers stay in Russian; only these instructions are English. Match the tone and terseness of the existing documents: concise, technical, file-referenced, no marketing language.
...
If this spec grew out of a `docs/backlog/backlog.md` row, add a pointer to this spec's path in that row — the trace-forward convention that keeps idea and spec linked in both directions. The row is deleted when the work is done, not marked "готово": that file is edited by ratchet.
