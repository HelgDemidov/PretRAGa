...
Paths: memory at the harness-provided project memory directory (`~/.claude/projects/<project-slug>/memory/` — the session states the resolved path; do not hard-code one, it differs per machine and per checkout); audit working files at `docs/memory/memory_sync/` (worklist, last-run record, backup archive) — this directory does not exist until the first run creates it, and its absence is the signal that no audit has ever happened.

## 1. Sources of truth (descending priority)

1. **Live code:** `src/`, `tools/`, `tests/`, any controlled-vocabulary data files (none exist yet — `CLAUDE.md` §4 says when to introduce one), the build and tool configuration (`pyproject.toml`, `uv.lock`, `schema.lock.json`), `.github/workflows/ci.yml`, `.gitignore`, and the command definitions themselves (`.claude/commands/`, untracked).
