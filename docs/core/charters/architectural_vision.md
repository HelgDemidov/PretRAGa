...
| Семь слоёв §5 — хранилища и потоки, порядок нестрогий | Называть их «стадиями конвейера» / строгим последовательным порядком выполнения | Данные пересекают границы слоёв (`ProvenanceAnchor` рождается в конвертации, живёт в запросах); триаж ветвится; приём нетерминален (`--recheck` — поперёк, не стадия потока); G2AI_ME эмпирически: независимо построенные модули требовали непрерывного шов-хардненинга на каждой границе |
